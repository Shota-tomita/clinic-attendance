...
const navItems: NavItem[] = [
  ...
  { href: '/attendance/correction',   label: '打刻修正申請',         icon: '📝', roles: ['admin','leader','staff'] },
  { href: '/attendance/early-start',  label: '早出申請',             icon: '⏰', roles: ['admin','leader','staff'] },
  { href: '/attendance/holiday-work', label: '休日出勤申請',         icon: '🏖️', roles: ['admin','leader','staff'] },
  { href: '/attendance/delay-request', label: '電車遅延申請',        icon: '🚃', roles: ['admin','leader','staff'] },
  ...
  { href: '/admin/corrections',       label: '打刻修正管理',         icon: '🔧', roles: ['admin'] },
  { href: '/admin/early-start',       label: '早出申請管理',         icon: '⏰', roles: ['admin'] },
  { href: '/admin/holiday-work',      label: '休日出勤申請管理',     icon: '🏖️', roles: ['admin'] },
  { href: '/admin/delay-requests',    label: '電車遅延申請管理',     icon: '🚃', roles: ['admin'] },
    ...
    return filteredNav.map(item => {
      ...
      return (
        <div key={item.href}>
          ...
          <Link href={item.href} onClick={onClose} className={`sidebar-link ${isActive ? 'active' : ''}`}>
    ...
    <div className="flex flex-col h-full">
      ...
      {isAdmin && (
        <div className="px-3 py-2 border-t border-gray-100">
          <Link href="/kiosk" target="_blank" className="sidebar-link text-clinic-600 hover:bg-clinic-50">
