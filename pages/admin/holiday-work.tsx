...
export default function AdminHolidayWorkPage() {
  ...
  const openReview = (req: any) => {
    ...
    setReviewStartTime(req.start_time?.slice(0, 5) ?? '')
    setAdminNote('')
  }

  const handleApprove = async (req: any) => {
    if (!reviewStartTime) { alert('始業時刻を入力してください'); return }
    setSaving(true)
  ...
  const handleDirectRegister = async () => {
    if (!directForm.user_id || !directForm.date || !directForm.start_time) {
      alert('スタッフ・日付・始業時刻を入力してください')
      return
    }
    setSaving(true)
    await supabase.from('holiday_work_requests').upsert({
    ...
    <Layout>
      <div className="max-w-3xl mx-auto space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-semibold text-gray-900">🏖️ 休日出勤申請管理</h1>
        ...
        <p className="text-xs text-gray-400 -mt-3">
          承認すると、その日は始業時刻から退勤までの全時間が残業として計算されます。
        </p>
        ...
        return (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 space-y-4">
              <h2 className="font-semibold text-gray-800">休日出勤申請の審査</h2>
              ...
              <div>
                <label className="label">始業時刻（この時刻から全て残業として計算） <span className="text-red-400">*</span></label>
                <input type="time" className="input" value={reviewStartTime}
      ...
      {showDirectForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 space-y-4">
            <h2 className="font-semibold text-gray-800">休日出勤を直接登録（承認済）</h2>
            <div>
            ...
            <div>
              <label className="label">始業時刻</label>
